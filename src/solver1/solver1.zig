...
const custom_debug = struct{
    fn debugPrintArray(comptime T : type, arr: [] const T) void{
        ...
        var k : u8 = 0; while(k<arr.len) : (k+=1) {
...
const BoardPos = struct{
    ...
    fn initBoard() [NUM_SQUARES] BoardPos{
       ...
       var k : u8 = 0; while(k<NUM_SQUARES) : (k+=1){
...
const MASK_MAP : [NUM_SIDE+1]Mask = if(NUM_SIDE == 9) [_]Mask{0,1,2,4,8,16,32,64,128,256} 
    else if(NUM_SIDE==16) [_]Mask{0,1,2,4,8,16,32,64,128,256,512,1024,2048,4096, 8192, 16384, 32768} 
    else unreachable;
...
const SolverBoard = struct{
    ...
    fn getMask(self: @This(), index: BoardIndex) Mask{
        ...
        return self.rowMasks[boardPos.row] 
            | self.colMasks[boardPos.col] 
            | self.boxMasks[boardPos.box];
    }
    ...
    fn insertValue(self : *@This(), index : BoardIndex, value: BoardValue) bool{
        ...
        const conflictDetected = ((valueMask & rowMask.*) != 0)
            or ((valueMask & colMask.*) != 0)
            or ((valueMask & boxMask.*) != 0);
...
const SudokuSolver = struct{
        ...
        if(grpSingleMask != MASK_EMPTY){
            ...
            while(k<NUM_SIDE) : (k+=1){
                ...
                if(isect != MASK_EMPTY){
                    ...
                    const changed = switch(isSingleCandidate) {
                        true => self.b.insertValueFromMask(idx,isect),
                        else => self.b.insertValue(idx,popFirstFromMask(isect)),
                    };
    ...
    fn startBranch(self: *@This()) SolverResult{
        ...
        self.candidateMasksByIndex[minIdx] = reducedMask;
        std.debug.assert((MASK_MAP[value] & reducedMask) == MASK_EMPTY);
        return SolverResult.NEED_RESTRICT;
    }
